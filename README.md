...
The resulting kernel demonstrates **improved classification accuracy and lower misclassification error** in several experiments compared to standard Gaussian RBF, sigmoid, and ReLU functions. :contentReference[oaicite:2]{index=2}
...
When the additional parameter vanishes, the kernel reduces to the **standard Gaussian RBF kernel**. :contentReference[oaicite:3]{index=3}
...
Experiments in the paper show that the generalized kernel can outperform several traditional activation functions in classification tasks. :contentReference[oaicite:4]{index=4}
